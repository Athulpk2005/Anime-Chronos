...
export default async function SeriesPage({
    ...
    let apiUrl = 'https://api.jikan.moe/v4/top/anime?type=tv&limit=24';
    if (genre) {
        apiUrl = `https://api.jikan.moe/v4/anime?type=tv&genres=${encodeURIComponent(genre)}&order_by=score&sort=desc&limit=24`;
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            const seen = new Set();
            series = data.filter((anime: Anime) => {
        ...
        console.error("Failed to fetch series from Jikan", e);
    }

    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-4xl font-black text-white tracking-tighter mb-2">Anime Series</h1>
                <p className="text-slate-400">Binge-watch the most popular episodic adventures.</p>
            </div>

            {/* Category Filter */}
            <CategoryFilter type="series" selectedGenre={genre} />
                ...
                {series.map((anime) => (
                    <Link href={`/anime/${anime.mal_id}`} key={anime.mal_id} className="group cursor-pointer">
                        ...
                        <div className="flex justify-between items-center mt-1">
                            ...
                            <span className="text-slate-400 text-[10px] font-bold">{anime.episodes ? `${anime.episodes} EP` : 'Ongoing'}</span>
                        </div>
                    </Link>
                ))}
            </div>
